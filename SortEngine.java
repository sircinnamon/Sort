...
public class SortEngine
{
	...
	public static void main(String[] args)
	{
		...
		for(String s : modes)
		{
			...
			if(s.equals("cyclesort")||s.equals("cycle")||s.equals("c"))
			{
				Cyclesort x = new Cyclesort(Arrays.copyOf(data,data.length),verbose,time);
				sortedData = x.sort();
				sequence = x.getSequence();
				if(time){t = x.getTime();}
			}
			if(s.equals("librarysort")||s.equals("library")||s.equals("lib"))
			{
				Librarysort x = new Librarysort(Arrays.copyOf(data,data.length),verbose,time);
				sortedData = x.sort();
				sequence = x.getSequence();
				if(time){t = x.getTime();}
			}
